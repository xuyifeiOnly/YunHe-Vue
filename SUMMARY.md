...
- **监控大屏** — 全屏实时监控中心，ScreenAdapter 等比自适应 + 深蓝科技感背景（网格 + 光晕 + 粒子动效）；顶部实时时钟 + 天气 + 系统名称品牌区；6 项 KPI 卡片（在线用户 / 总用户 / 今日登录 / 帧率 / 运行时间 / Redis Key）；左侧浏览器 / OS 占比双饼图；中央中国地图 ECharts + 散点闪烁 + 飞线动效（GeoJSON 动态注册）；右侧 4 组仪表盘（CPU / 内存 / 磁盘 / 负载）+ Redis 命令 TOP5 柱状图；底部在线用户列表 + 登录记录双表格自动滚动
...
- **IconSelect** — 图标选择器
- **ScreenAdapter** — 大屏自适应容器，基于 CSS transform scale 等比缩放 + 居中偏移，设计稿宽度/高度可配置，移动端自动跳过缩放，防抖 + requestAnimationFrame 60 帧流畅适配
