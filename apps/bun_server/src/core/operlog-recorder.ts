...
export async function recordOperlog(
  ...
  const skipTransform =
    routeMeta.skipTransform ||
    response?.headers.get('content-type')?.includes('text/event-stream') ||
    response?.headers.get('content-disposition')
  const status =
    success || skipTransform
  ...
  record.url = path
  record.businessType = (routeMeta.operLog.businessType ??
  ...
  await context.services.logService
    .createOperlog(record)
    .catch((error: unknown) => logError('操作日志记录失败', error))
}
