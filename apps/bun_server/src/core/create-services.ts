...
export function createServices(
  ...
  const dictService = new DictService(
    redisService,
    dataSource.getRepository(DictTypeEntity),
    dataSource.getRepository(DictDataEntity),
  )
  ...
  const jobService = new JobService(
    ...
    excelService,
    dataSource.getRepository(JobEntity),
    dataSource.getRepository(JobLogEntity),
  )
  const logService = new LogService(
    excelService,
    dataSource.getRepository(LogininforEntity),
    dataSource.getRepository(OperLogEntity),
  )
  const onlineService = new OnlineService(redisService)
  const serverService = new ServerService()
  const aiService = new AiService(
    config,
    dataSource.getRepository(AiMessageEntity),
    dataSource.getRepository(AiConversationEntity),
  )
  const promptService = new PromptService(
    dataSource.getRepository(PromptEntity),
  )
