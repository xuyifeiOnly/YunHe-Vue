...
export function assertRecord(
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new BusinessException(`${fieldName}必须为对象`)
...
export function parseString(
  ...
  if (options.max !== undefined && result.length > options.max)
    throw new BusinessException(`${fieldName}长度不能超过${options.max}个字符`)
  return result
}
...
export function parsePagination(
  ...
  if (pageSize > maxPageSize)
    throw new BusinessException(`每页条数不能超过${maxPageSize}`)
  return {
    pageNo,
    pageNum: pageNo,
    pageSize,
...
export function parseIds(
  ...
  const ids = Array.isArray(value)
    ...
    : String(value ?? '')
        ...
        .map((item) => item.trim())
        .filter(Boolean)
  if (options.required !== false && !ids.length)
    throw new BusinessException(`${fieldName}不能为空`)
  return ids
}
...
export function idsFrom(input: IdsPayload = {}, fieldName = 'ID') {
  return parseIds(
    input.ids ?? input.jobIds ?? input.logIds ?? input.id,
    fieldName,
  )
}
