...
export class OnlineService {
  ...
  public async onlineList(query: QueryParams = {}) {
    ...
    const tokenKeys = await this.redisService.scanKeys(
      `${RedisConstant.ACCESS_TOKEN_KEY}:*`,
    )
    const records = (
      await Promise.all(
        tokenKeys.map(async (key) => {
          ...
          const value = await this.redisService.get(
            `${RedisConstant.ADMIN_USER_ONLINE_KEY}:${id}`,
          )
      ...
      (item) =>
        (!query.username ||
          String(item.username ?? '').includes(query.username)) &&
        (!query.ip || String(item.ip ?? '').includes(query.ip)) &&
        (!query.location ||
          String(item.location ?? '').includes(query.location)),
    )
  ...
  public async onlineCount() {
    return (
      await this.redisService.scanKeys(`${RedisConstant.ACCESS_TOKEN_KEY}:*`)
    ).length
  }
  ...
  public async forceLogout(data: { userId?: string; uuid?: string }) {
    ...
    await this.redisService.del(
      `${RedisConstant.ACCESS_TOKEN_KEY}:${data.userId}:${data.uuid}`,
      `${RedisConstant.ADMIN_USER_ONLINE_KEY}:${data.userId}:${data.uuid}`,
    )
    return '强退成功'
  }
  ...
  private async normalizeOnlineUser(item: OnlineUserRecord) {
    ...
    const location =
      item.location ??
      item.loginLocation ??
      (ip ? await getLocationByIP(ip).catch(() => '') : '')
    return {
      ...item,
