...
const EXPORT_COLUMNS: Record<string, ExcelColumn[]> = {
  logininfor: [
    ...
    { key: 'loginTime', header: '登录时间', width: 22, formatter: formatDateValue },
  ],
  operlog: [
    { key: 'title', header: '系统模块' },
    { key: 'businessType', header: '业务类型', dict: { '0': '其它', '1': '新增', '2': '修改', '3': '删除', '4': '导出', '5': '导入' } },
    ...
    { key: 'operTime', header: '操作时间', width: 22, formatter: formatDateValue },
  ],
  joblog: [
    ...
    { key: 'createTime', header: '执行时间', width: 22, formatter: formatDateValue },
  ],
  job: [
    { key: 'jobName', header: '任务名称' },
    { key: 'jobGroup', header: '任务组名' },
    ...
    { key: 'misfirePolicy', header: '错误策略', dict: { '1': '立即执行', '2': '执行一次', '3': '放弃执行' } },
    { key: 'concurrent', header: '并发执行', dict: { '0': '禁止', '1': '允许' } },
    { key: 'status', header: '任务状态', dict: JOB_STATUS_DICT },
    { key: 'createTime', header: '创建时间', width: 22, formatter: formatDateValue },
  ],
}

export class ExcelService {
  public async exportBuffer(records: Record<string, unknown>[], sheetName = 'Sheet1', columns?: ExcelColumn[]) {
    const workbook = new ExcelJS.Workbook()
    const worksheet = workbook.addWorksheet(sheetName, { views: [{ state: 'frozen', ySplit: 1 }] })
    const finalColumns: ExcelColumn[] = columns?.length ? columns : this.getColumns(records)
    worksheet.columns = finalColumns.map((column) => ({ header: column.header, key: column.key, width: column.width ?? 20 }))
    worksheet.addRows(records.map((record) => this.mapRecord(record, finalColumns)))
  ...
  public async exportResponse(records: Record<string, unknown>[], filename: string, columnsOrType?: ExcelColumn[] | keyof typeof EXPORT_COLUMNS) {
    const columns = Array.isArray(columnsOrType) ? columnsOrType : columnsOrType ? EXPORT_COLUMNS[columnsOrType] : this.inferColumns(filename)
    const buffer = await this.exportBuffer(records, filename.replace(/\.xlsx$/i, '').slice(0, 31) || 'Sheet1', columns)
  ...
  public async importBuffer(file: Buffer | ArrayBuffer, columns: ExcelColumn[]): Promise<Record<string, unknown>[]> {
    const workbook = new ExcelJS.Workbook()
    ...
    for (const col of columns) if (col.dict) reverseDictMap.set(col.key, Object.fromEntries(Object.entries(col.dict).map(([k, v]) => [v, k])))
    ...
    worksheet.eachRow((row, rowNum) => {
      ...
      row.eachCell((cell, colNumber) => {
        ...
        if (typeof value === 'object' && value !== null && 'text' in value) value = String((value as { text: string }).text)
        const reverseDict = reverseDictMap.get(key)
        if (reverseDict && typeof value === 'string') value = reverseDict[value] ?? value
        record[key] = value
      })
      rows.push(record)
    })
  ...
  public async importFromRequest(file: File, columns: ExcelColumn[]): Promise<Record<string, unknown>[]> {
    const buffer = await file.arrayBuffer()
    return this.importBuffer(buffer, columns)
  }

  /** 生成导入模板 */
  public async importTemplateResponse(filename: string, columns: ExcelColumn[], sheetName = 'Sheet1') {
    const workbook = new ExcelJS.Workbook()
    const worksheet = workbook.addWorksheet(sheetName, { views: [{ state: 'frozen', ySplit: 1 }] })
  ...
  private mapRecord(record: Record<string, unknown>, columns: ExcelColumn[]) {
    ...
    for (const column of columns) {
      ...
      row[column.key] = column.formatter ? column.formatter(value, record) : column.dict ? column.dict[String(value)] ?? value : value
    }
    return row
  }
  ...
  private getColumns(records: Record<string, unknown>[]) {
    ...
    for (const record of records) Object.keys(record).forEach((key) => keys.add(key))
