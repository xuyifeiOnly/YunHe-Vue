  ...
  return {
    ...
    totalChunks:
      data.totalChunks === undefined
        ? undefined
        : parseTotalChunks(data.totalChunks),
  }
}
...
const routes = [
  {
    method: 'POST',
    path: '/common/upload/file',
    description: '文件上传',
    repeatSubmit: false,
    handler: ({ body, services }: RouteContext<UploadFileBody>) =>
      services.uploadService.uploadFile(parseFile(body.file)),
  },
  {
    method: 'POST',
    path: '/common/upload/check',
    description: '上传检查',
    repeatSubmit: false,
    handler: ({ body, services }: RouteContext<CheckFileBody>) =>
      services.uploadService.checkFile(parseCheckBody(body)),
  },
  {
    method: 'POST',
    path: '/common/upload/chunk',
    description: '分片上传',
    repeatSubmit: false,
    handler: ({ body, services }: RouteContext<ChunkBody>) => {
      const data = parseChunkBody(body)
      return services.uploadService.uploadChunk(
        data.file,
        data.hash,
        data.index,
      )
  ...
  {
    method: 'POST',
    path: '/common/upload/chunk/merge',
    description: '分片合并',
    repeatSubmit: false,
    handler: ({ body, services }: RouteContext<MergeBody>) =>
      services.uploadService.mergeChunks(parseMergeBody(body)),
  },
  {
    method: 'DELETE',
    path: '/common/upload/chunk/clear',
    description: '分片清理',
    repeatSubmit: false,
