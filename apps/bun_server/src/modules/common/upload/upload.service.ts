...
export class UploadService {
  ...
  public async checkFile(data: { hash?: string; fileHash?: string; fileName?: string }) {
    ...
    const finalPath = finalName ? this.safeResolve(this.uploadRoot, finalName) : ''
    const chunkDir = hash ? this.safeResolve(this.uploadRoot, 'chunks', hash) : ''
    const isExist = Boolean(finalPath && existsSync(finalPath))
    return { isExist, exists: isExist, uploadedChunks: chunkDir && existsSync(chunkDir) ? readdirSync(chunkDir) : [], uploaded: chunkDir && existsSync(chunkDir) ? readdirSync(chunkDir) : [], url: isExist ? `/uploads/${finalName}` : '' }
  }

  public async uploadChunk(file: File | undefined, hash?: string, index?: string | number, chunkHash?: string) {
    if (!file || !hash) throw new BusinessException('分片参数不完整')
    this.validateHash(hash)
    const chunkName = this.getSafeChunkName(chunkHash ?? String(index ?? file.name))
    const chunkDir = this.safeResolve(this.uploadRoot, 'chunks', hash)
    mkdirSync(chunkDir, { recursive: true })
    await writeFile(this.safeResolve(chunkDir, chunkName), Buffer.from(await file.arrayBuffer()))
    return '分片上传成功'
  }

  public async mergeChunks(data: { hash?: string; fileHash?: string; fileName: string; totalChunks?: number | string }) {
    const hash = data.fileHash ?? data.hash
    ...
    if (!existsSync(chunkDir)) throw new BusinessException('分片文件不存在，请重新上传')
    const chunks = this.getOrderedChunks(chunkDir, data.totalChunks)
    const writeStream = createWriteStream(finalFilePath)
    writeStream.setMaxListeners(0)
    for (const chunk of chunks) await pipeline(createReadStream(this.safeResolve(chunkDir, chunk)), writeStream, { end: false })
    writeStream.end()
  ...
  public async clearChunk(hash?: string) {
    if (hash) {
      this.validateHash(hash)
      await rm(this.safeResolve(this.uploadRoot, 'chunks', hash), { recursive: true, force: true })
    }
    return '清理成功'
  }
  ...
  private getOrderedChunks(chunkDir: string, totalChunks?: number | string) {
    const chunks = readdirSync(chunkDir).map((item) => this.getSafeChunkName(item))
    if (!chunks.length) throw new BusinessException('分片文件为空，请重新上传')
    const ordered = chunks.sort((a, b) => Number(a.split('-').pop() ?? a) - Number(b.split('-').pop() ?? b))
    if (totalChunks !== undefined && totalChunks !== '') {
      const total = Number(totalChunks)
      if (!Number.isInteger(total) || total <= 0) throw new BusinessException('分片总数格式错误')
      if (ordered.length !== total) throw new BusinessException('分片数量不完整，请重新上传')
    }
    if (ordered.every((item) => /^\d+$/.test(item))) {
      ordered.forEach((item, index) => {
        if (Number(item) !== index) throw new BusinessException('分片序号不连续，请重新上传')
      })
    }
    return ordered
  }
  ...
  private getSafeChunkName(name: string) {
    if (!/^\d+$/.test(name) && !/^[a-fA-F0-9_-]{1,128}$/.test(name)) throw new BusinessException('分片序号格式错误')
    return name
  }
  ...
  private safeResolve(root: string, ...paths: string[]) {
    ...
    if (targetPath !== rootPath && !targetPath.startsWith(`${rootPath}/`)) throw new BusinessException('文件路径非法')
    return targetPath
  }
