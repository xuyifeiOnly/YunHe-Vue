...
import { AiConversationEntity, AiMessageEntity, BusinessException, CommonConstant } from '../../common'
import type { AppConfig } from '../../config/config'
import { buildSummaryPrompt, SYSTEM_PROMPT } from './ai.prompt'
import { runEnabledAiTools } from './ai.tools'

type ChatData = { message?: string; prompt?: string; conversationId?: string; enableInternetSearch?: boolean }
...
export class AiService {
  ...
  public async chat(data: ChatData, userId: string) {
    ...
    if (!this.model) {
      ...
      await this.saveMessage(ctx.conversationId, 'assistant', content, estimateTokens(content))
      return { content, conversationId: ctx.conversationId }
    }
    const messages = await this.buildContextMessages(ctx.conversationId)
    await this.appendToolResults(messages, ctx.userContent, data.enableInternetSearch)
  ...
  public stream(data: ChatData, userId: string) {
    ...
    const write = (payload: unknown) => encoder.encode(`data: ${JSON.stringify(payload)}\n\n`)
    return new Response(
      new ReadableStream({
        start: async (controller) => {
          ...
          try {
            ...
            await this.saveMessage(conversationId, 'assistant', fullReply, estimateTokens(fullReply))
            await this.lazyUpdateSummary(conversationId)
            controller.enqueue(write({ status: 'DONE', conversationId }))
            controller.close()
          } catch {
            controller.enqueue(write({ status: 'ERROR', message: 'AI 对话失败', conversationId }))
            controller.close()
          }
        },
      }),
  ...
  public async getConversations(userId: string) {
    return this.conversationRepository.find({ where: { userId }, order: { createTime: 'DESC' } })
  }

  public async deleteConversation(conversationId: string, userId: string) {
    const conversation = await this.conversationRepository.findOneBy({ id: conversationId, userId })
  ...
  public async updateConversationTitle(data: { conversationId?: string; id?: string; title?: string }, userId: string) {
    const conversationId = data.conversationId ?? data.id
    if (!conversationId || !data.title) throw new BusinessException('参数不完整')
    const conversation = await this.conversationRepository.findOneBy({ id: conversationId, userId })
    if (!conversation) throw new BusinessException(`会话 ${conversationId} 不存在`)
  ...
  public async getMessages(conversationId: string, userId: string) {
    const conversation = await this.conversationRepository.findOneBy({ id: conversationId, userId })
    if (!conversation) throw new BusinessException(`会话 ${conversationId} 不存在`)
    return this.messageRepository.find({ where: { conversationId }, order: { createTime: 'ASC' } })
  }
  ...
  private async prepareChatContext(data: ChatData, userId: string) {
    ...
    const conversationId = await this.ensureConversation(data.conversationId, userId, userContent)
    await this.saveMessage(conversationId, 'user', userContent, estimateTokens(userContent))
    return { conversationId, userContent }
  }

  private async ensureConversation(conversationId: string | undefined, userId: string, userContent: string) {
    if (conversationId) {
      const conversation = await this.conversationRepository.findOneBy({ id: conversationId, userId })
      if (!conversation) throw new BusinessException(`会话 ${conversationId} 不存在`)
  ...
  private async buildContextMessages(conversationId: string) {
    ...
    const latestMessages = await this.messageRepository.find({ where: { conversationId }, order: { createTime: 'DESC' }, take: this.RECENT_MESSAGE_COUNT })
    const messages: BaseMessage[] = [new SystemMessage(this.systemPrompt)]
    if (conversation?.summary) messages.push(new SystemMessage(`历史摘要：${conversation.summary}`))
  ...
  private async lazyUpdateSummary(conversationId: string) {
    ...
    if (count < this.SUMMARY_TRIGGER_COUNT || (count - this.SUMMARY_TRIGGER_COUNT) % this.SUMMARY_TRIGGER_COUNT !== 0) return
  ...
  private async generateSummary(conversationId: string) {
    ...
    const recent = await this.messageRepository.find({ where: { conversationId }, order: { createTime: 'DESC' }, take: this.SUMMARY_TRIGGER_COUNT })
    const newText = recent.reverse().map((item) => `${item.role}: ${item.content}`).join('\n')
    if (!this.model) return `${conversation?.summary ? `${conversation.summary}\n` : ''}${newText}`.slice(-2000)
    const template = buildSummaryPrompt(Boolean(conversation?.summary))
    return ChatPromptTemplate.fromTemplate(template).pipe(this.model).pipe(new StringOutputParser()).invoke({ existingSummary: conversation?.summary ?? '', newText })
  }

  private async appendToolResults(messages: BaseMessage[], content: string, enabled?: boolean) {
    if (!enabled) return
