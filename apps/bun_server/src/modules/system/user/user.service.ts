...
export class UserService {
  ...
  public async create(data: CreateUserBody) {
    validate(data, [
      ...
      { field: 'email', label: '邮箱', validator: (v) => (v && !isValidEmail(String(v)) ? '邮箱格式不正确' : undefined) },
    ])
  ...
  public async update(data: UpdateUserBody) {
    ...
    if (!user) throw new BusinessException('该用户不存在')
    if (user.id === CommonConstant.ADMIN_USER_ID && data.status === CommonConstant.STATUS_DISABLE) throw new BusinessException('管理员账号不能停用')
    await this.validateUserFields(data, data.id)
    Object.assign(user, data)
    if (data.roleIds) user.roles = data.roleIds.length ? await this.roleRepository.findBy({ id: In(data.roleIds) }) : []
    await this.userRepository.save(user)
  ...
  public async updatePassword(userId: string, updateDto: UpdateUserPasswordBody) {
    ...
    if (!oldPassword || !newPassword) throw new BusinessException('旧密码和新密码不能为空')
    ...
    if (!(await verifyPassword(oldPassword, user.password))) throw new BusinessException('旧密码错误')
    if (await verifyPassword(newPassword, user.password)) throw new BusinessException('新密码不能与原密码相同')
  ...
  public async getProfile(userId: string) {
    ...
    const roles = await this.roleRepository.find({ where: { status: CommonConstant.STATUS_NORMAL }, order: { roleSort: 'ASC' } })
  ...
  public async resetPassword(data: ResetUserPasswordBody) {
    ...
    const user = await this.userRepository.findOne({ where, select: { id: true, password: true } })
    if (!user) throw new BusinessException('该用户不存在')
    if (await verifyPassword(data.password, user.password)) throw new BusinessException('新密码不能与原密码相同')
    await this.userRepository.update(user.id, { password: await encryptPassword(data.password) })
  ...
  public async cleanUserRelatedCache(userId: string) {
    await this.redisService.del(`${RedisConstant.ADMIN_USER_ROLES}:${userId}`, `${RedisConstant.ADMIN_USER_PERMISSIONS}:${userId}`)
  }

  public async cleanUserAllCache(userId: string) {
    const onlineKeys = await this.redisService.scanKeys(`${RedisConstant.ADMIN_USER_ONLINE_KEY}:${userId}:*`)
    const tokenKeys = await this.redisService.scanKeys(`${RedisConstant.ACCESS_TOKEN_KEY}:${userId}:*`)
    const keys = [`${RedisConstant.ADMIN_USER_ROLES}:${userId}`, `${RedisConstant.ADMIN_USER_PERMISSIONS}:${userId}`, ...onlineKeys, ...tokenKeys]
  ...
  private async validateUserFields(data: { id?: string; username?: string; phone?: string; email?: string }, userId?: string) {
    if (data.username) {
      ...
      if (await this.userRepository.existsBy(where)) throw new BusinessException('该用户已存在')
    }
    if (await this.checkPhoneExists(data.phone, userId)) throw new BusinessException('该手机号已存在')
    if (data.email) {
      const where: FindOptionsWhere<UserEntity> = { email: Equal(data.email) }
      if (userId) where.id = Not(userId)
      if (await this.userRepository.existsBy(where)) throw new BusinessException('该邮箱已存在')
    }
  }
