...
export class DictService {
  ...
  public async updateType(data: Partial<DictTypeEntity>) {
    ...
    if (data.dictType && data.dictType !== oldType.dictType)
      await this.dictDataRepository.update(
        { dictType: oldType.dictType },
  ...
  public async deleteType(ids: string) {
    ...
    for (const type of types) {
      const count = await this.dictDataRepository.countBy({
        dictType: type.dictType,
      })
      if (count > 0)
        throw new BusinessException(
          `字典类型 ${type.dictName} 已分配字典数据，不能删除`,
        )
    }
  ...
  public async findDataByType(dictType: string) {
    ...
    const list = await this.dictDataRepository.find({
      where: { dictType, status: CommonConstant.STATUS_NORMAL },
      order: { dictSort: 'ASC' },
    })
  ...
  public async clearCache(...dictTypes: string[]) {
    const patterns = dictTypes.length
      ? dictTypes.map((type) => `${RedisConstant.DICTTYPE_KEY}:${type}`)
      : [`${RedisConstant.DICTTYPE_KEY}:*`]
    const keys = (
      await Promise.all(
  ...
  private async validateType(data: Partial<DictTypeEntity>, id?: string) {
    if (!data.dictName || !data.dictType)
      throw new BusinessException('字典名称和类型不能为空')
    const where: FindOptionsWhere<DictTypeEntity> = {
      dictType: Equal(data.dictType),
    }
    if (id) where.id = Not(id)
    if (await this.dictTypeRepository.existsBy(where))
      throw new BusinessException(`字典类型 ${data.dictType} 已存在`)
  }
  ...
  private async validateData(data: Partial<DictDataEntity>) {
    if (
      ...
      throw new BusinessException('字典类型、标签和值不能为空')
    const type = await this.dictTypeRepository.findOneBy({
      dictType: data.dictType,
    })
