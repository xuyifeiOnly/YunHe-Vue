...
export class MenuService {
  ...
  public async findRoutesByRoleIds(roleIds: string[], isAdmin: boolean) {
    ...
    queryBuilder.andWhere('menu.status = :status', {
  ...
  public async findList(queryParams: {
    status?: string
    menuName?: string
    menuType?: string
  }) {
  ...
  private async validateMenu(data: Partial<MenuEntity>, id?: string) {
    ...
    if (data.parentId && data.parentId !== CommonConstant.DEFAULT_PARENT_ID) {
      ...
      if (parent.menuType === 'F')
        throw new BusinessException('父级菜单不能为按钮')
    ...
    if (data.path) {
      ...
      if (await this.menuRepository.existsBy(where))
        throw new BusinessException(`路由地址 ${data.path} 已存在`)
    }
    if (data.permission) {
      const where: FindOptionsWhere<MenuEntity> = {
        permission: Equal(data.permission),
      }
      if (id) where.id = Not(id)
      if (await this.menuRepository.existsBy(where))
        throw new BusinessException(`权限字符 ${data.permission} 已存在`)
    }
  }
}
